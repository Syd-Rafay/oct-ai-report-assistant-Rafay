...
export async function POST(request: NextRequest) {
  const env = requiredGatewayEnv(["RETINA_DR_GRADCAM_BACKEND_URL", "RETINA_GRADCAM_BACKEND_URL"]);
  if (!env) return jsonError("Retina DR Grad-CAM gateway is not configured.", 500);
